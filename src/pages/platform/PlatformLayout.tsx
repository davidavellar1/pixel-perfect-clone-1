    ...
    <div className={cn("grid min-h-screen bg-background transition-[grid-template-columns] duration-[220ms] ease-out", collapsed ? "grid-cols-[74px_minmax(0,1fr)]" : "grid-cols-[264px_minmax(0,1fr)]")}>
      <aside className="sticky top-0 z-40 flex h-screen min-w-0 flex-col overflow-hidden border-r border-sidebar-border bg-sidebar">
        <div className="flex h-[68px] shrink-0 items-center justify-between border-b border-sidebar-border px-4">
          <Link to="/app/opportunities" className="min-w-0 font-display text-xl font-bold text-foreground" title="DHC Market">
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-3 pb-4">
          {groups.map((group) => <div key={group.label}>
            {!collapsed && <p className="px-3 pb-2 pt-5 text-[10px] font-semibold uppercase text-muted-foreground">{group.label}</p>}
        ...
        <div className="shrink-0 border-t border-sidebar-border p-3">
          <div className={cn("mb-3 flex items-center", collapsed ? "justify-center" : "gap-3 px-1")}>
            ...
            {!collapsed && <div className="min-w-0"><p className="truncate text-sm font-medium text-foreground">{displayName}</p><p className="truncate text-xs text-muted-foreground">{company}</p></div>}
          </div>
          <Button variant="ghost" title="Sign out" onClick={handleSignOut} className={cn("w-full text-muted-foreground", collapsed ? "px-0" : "justify-start")}><LogOut className="h-4 w-4" />{!collapsed && "Sign out"}</Button>
        </div>
      </aside>
